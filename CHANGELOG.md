...
- 주제 레코드에 `contentKind`(`mechanical-derivative` | `source-grounded-draft`), `misconceptions`, `contentSourceLocator`를 추가했습니다. `data/kr/manifest.json`은 오버레이 파일 수와 출처 기반 초안 주제 수를 `counts.contentOverlayFiles` 22 · `counts.sourceGroundedTopics` 1,956에 기록합니다.
...
- **나머지 8개 교과 오버레이 14개** — 사회 147 / 영어 120 / 도덕 120 / 실과 78 / 음악 78 / 체육 147 / 미술 78 / 통합교과 171(바른·슬기로운 생활은 2022-33 별책 15, 즐거운·건강한 생활은 2026-1 별책 15 근거). 각 별책의 성취기준 해설·적용 시 고려 사항을 근거로 하며 해설이 없는 항목은 `note`에 명시했습니다.
- 출처 기반 초안 주제는 **1,956개 전부(100%)**이고 `mechanical-derivative` 주제는 0개입니다. 오버레이 22개 파일 사이의 evidence·prompt 완전 중복은 0건입니다.
- 오버레이 검증기의 관찰 가능 증거 판정은 기계 repair 경로(`isLearnerObservableEvidence`)와 집필 경로(`isAuthoredObservableEvidence`)로 나뉘며, 집필 경로만 현재형 행동 종결형을 받아들이고 "출처·원문" 낱말은 로케이터 문맥일 때만 provenance로 봅니다.
- 초등 사회의 `.evidence` 접미사 주제 49개는 `type`이 REPRESENTATIONAL이면 `representation`으로 사상해 한 성취기준 안에서 facetKey가 유일하도록 했습니다(검증기 검사 추가, 주제 ID 불변). 과학 `focus` 필드의 PDF 줄바꿈 공백 6건을 정정했습니다.
