...
| CQ-11 | How many source-data coverage gaps are retained, and how are they categorized? | Release fixture reports 43 `CoverageGap` resources. The separate ontology-format field reports `p3-formal-release`; it is not counted as gap 44. |
| CQ-12 | Is the source-rights status independently discoverable from coverage and format status? | Release and source-document metadata return `RightsStatus/cleared` and the public-government-document basis; the query does not return it as a `CoverageGap`. |
